...
export class globject {
    ...
    glinit(gl) {
        ...
        this.lb = gl.getUniformLocation(this.shader.program, "lb");
        this.ub = gl.getUniformLocation(this.shader.program, "ub");
    ...
    draw(gl) {
        ...
        gl.uniform3fv(this.lb, [this.mesh.collision_box[0], this.mesh.collision_box[1], this.mesh.collision_box[2]]);
        gl.uniform3fv(this.ub, [this.mesh.collision_box[3], this.mesh.collision_box[4], this.mesh.collision_box[5]]);
    ...
    collision(obj) {
        ...
        let t1 = [(this.mesh.collision_box[0] + this.mesh.collision_box[3]) / 2, 
            (this.mesh.collision_box[1] + this.mesh.collision_box[4]) / 2,
            (this.mesh.collision_box[2] + this.mesh.collision_box[5]) / 2]
    
        let t2 = [(obj.mesh.collision_box[0] + obj.mesh.collision_box[3]) / 2, 
            (obj.mesh.collision_box[1] + obj.mesh.collision_box[4]) / 2,
            (obj.mesh.collision_box[2] + obj.mesh.collision_box[5]) / 2]

        let t3 = [this.mesh.collision_box[0], this.mesh.collision_box[1], this.mesh.collision_box[2]]
        let t4 = [this.mesh.collision_box[3], this.mesh.collision_box[4], this.mesh.collision_box[5]]
        // for(let i = 0; i < 3; i++) cb1[i + 3] = cb1[i + 3] - cb1[i]
        
        let t5 = [obj.mesh.collision_box[0], obj.mesh.collision_box[1], obj.mesh.collision_box[2]]
        let t6 = [obj.mesh.collision_box[3], obj.mesh.collision_box[4], obj.mesh.collision_box[5]]


        t1 = globject.to_xyz(vec3.transformMat4(t1, t1, this.worldMatrix))
        t2 = globject.to_xyz(vec3.transformMat4(t2, t2, obj.worldMatrix))

        var l1 = globject.to_xyz(vec3.transformMat4(t3, t3, this.worldMatrix)),
            u1 = globject.to_xyz(vec3.transformMat4(t4, t4, this.worldMatrix)),
            l2 = globject.to_xyz(vec3.transformMat4(t5, t5, obj.worldMatrix)),
            u2 = globject.to_xyz(vec3.transformMat4(t6, t6, obj.worldMatrix))

        if(this.id == "ground" || obj.id == "ground"){
            ...
            return Math.sqrt(Math.pow(t1.x - t2.x, 2.0) + Math.pow(t1.z - t2.z, 2.0)) <= (Math.abs(u1.x - l1.x) + Math.abs(u2.x - l2.x)) / 2;
